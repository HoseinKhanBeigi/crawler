...
import { LEGAL_LEAD_TABLE } from '../../store/settings/settings.constants';
...
import { LEGAL_LEAD } from './constant';
...
const Leads = props => {
  ...
  const externalDataConverter = data => {
    ...
    const columns = [
      {
        code: 'assignedTags',
        permission: getPermissionStatus(Actions.legalLeadUnTag),
        type: LEGAL_LEAD,
        name: 'برچسب',
        order: 1000,
      },
  ...
  const deselectAndRefreshTable = () => {
    deSelectRows();
    kianTableApi(LEGAL_LEAD_TABLE).refreshTable();
  };
  ...
  const contextMenu = [
    {
      ...
      authority: Actions.businessLeadUpdate,
      action: row =>
        showModal(DRAWER_FOR_VIEW_AND_EDIT_LEAD)({
          leadType: LEGAL_LEAD,
          tableIndex: LEGAL_LEAD_TABLE,
          levantId: row?.levantId,
          leadId: row?.leadId,
          title: 'مشاهده و ویرایش سرنخ حقوقی',
        })(),
    },
    {
      label: `ارسال پیام`,
      authority: Actions.legalLeadSendSms,
    ...
    {
      label: `ارسال ایمیل`,
      authority: Actions.legalLeadSendEmail,
      isNotAccess: statusIsNotAccess,
      action: row =>
    ...
    {
      label: `اختصاص دادن`,
      authority: Actions.legalLeadAssign,
      isNotAccess: statusIsNotAccess,
      action: row =>
    ...
    {
      label: `رفع مسئولیت`,
      authority: Actions.legalLeadUnAssign,
      isNotAccess: statusIsNotAccess,
      action: row =>
    ...
    {
      label: `برچسب زدن`,
      authority: Actions.legalLeadTag,
      isNotAccess: statusIsNotAccess,
      action: row =>
    ...
    {
      label: `تغییر وضعیت`,
      authority: Actions.legalLeadChangeStatus,
      isNotAccess: statusIsNotAccess,
      action: row =>
  ...
  const activityButton = [
    {
      label: 'افزودن',
      icon: 'user-add',
      authority: Actions.LeadCreateUserDefinedBusiness,
  ...
  const actionButtons = [
    ...
    },
    {
      ...
      icon: 'disconnect',
      authority: Actions.legalLeadUnTag,
      isNotAccess: statusGroupIsNotAccess,
  ...
  const downloadExcelAction = () => () => {
    showModal(MODAL_FOR_DOWNLOAD_TABLE_EXCEL)({
      endpoint: `leads/business/export/dynamically/excel`,
  ...
  return (
    <>
      <KianTable
        endpoint="leads/business/search"
        ...
        tableId={LEGAL_LEAD_TABLE}
        filterType="LEAD"
        headerTitle="سرنخ حقوقی"

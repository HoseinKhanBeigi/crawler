...
const CreditReportTab = props => {
  ...
  const gererateDate = d => {
    const acctulDate = `${cardDetailData?.checkDate?.slice(
      0,
      4,
    )}/${cardDetailData?.checkDate?.slice(
      4,
      6,
    )}/${cardDetailData?.checkDate?.slice(6, 8)}`;
    return acctulDate;
  };
  return (
    <>
        ...
        <Row>
          <Col span={16}>
            <Descriptions column={2}>
              ...
              >
                {momentJalaali(details?.reqInquiryDate).format(
                  'dddd jD jMMMM jYYYY ',
                )}
              </Descriptions.Item>
            </Descriptions>
